...
public class Server {
	...
	/*
	 * public static void main(String[] args) { // 현재 접속되어 있는 클라이언트 정보
	 * ArrayList<ServerChatter> chatters = new ArrayList<ServerChatter>();
	 * 
	 * // 서버소켓 객체 선언 ServerSocket serverSocket = null; Socket socket = null;
	 * 
	 * // 접속된 순서 번호 int no = 0; ServerChatter chatter = null; try { // 서버소켓 생성
	 * serverSocket = new ServerSocket(9002); while (true) {
	 * System.out.println("***********클라이언트 접속 대기중*************"); socket =
	 * serverSocket.accept();
	 * 
	 * // 채팅 객체 생성 chatter = new ServerChatter(socket, chatters, no);
	 * chatter.login(); // 대화명 입력 처리
	 * 
	 * // 채팅 객체를 ArrayList에 저장한다. chatters.add(chatter); no++;
	 * 
	 * // 접속된 순서에 따라 1대1 채팅을 시키기 위함 if (no % 2 == 0) { // 두명의 채터가 들어오면 쓰레드를 시작시킴
	 * chatters.get(no - 2).start(); chatters.get(no - 1).start(); } } } catch
